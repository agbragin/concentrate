...
.service('FilesystemBrowserService', function($http, $log, $rootScope) {
    ...
    let parseFolderResource = res => {
        ...
        let files = res.data['files'].map(it => new LocalFile(res.data['path'], it));
        let folderRefs = new Map();
        res.data['folders'].forEach(it => folderRefs.set(it, res.data['_links'][it]['href']));

        return new LocalFolder(res.data['path'], files, folderRefs, res.data['isFile']);
    };
